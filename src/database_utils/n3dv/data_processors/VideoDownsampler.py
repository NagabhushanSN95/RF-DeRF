...
def downsample_data(database_dirpath: Path, downsampling_factor: int, num_frames: int):
    for scene_dirpath in sorted(database_dirpath.iterdir()):
        # Downsample the RGB videos
        rgb_dirpath = scene_dirpath / 'rgb'
        rgb_down_dirpath = scene_dirpath / f'rgb_down{downsampling_factor}'
...
def demo1():
    ...
    num_frames = 300
    downsample_data(database_dirpath, downsampling_factor, num_frames)
    return
