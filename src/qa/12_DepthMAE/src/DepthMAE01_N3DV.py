...
import skimage.io
import os
if 'FFmpegPATH' in os.environ:
    import skvideo
    skvideo.setFFmpegPath(os.environ['FFmpegPATH'])
import skvideo.io
...
class DepthMAE:
    ...
    def read_depth(cls, depth_path: Path, depth_scales_path: Path, video_num: int):
        ...
        if (depth.shape[1] == 3042) and (depth.shape[2] == 1352):
            depth = depth[:, :1014]
...
def demo1():
    ...
    avg_score = start_qa(pred_train_dirpath, database_dirpath, gt_depth_dirpath, resolution_suffix, downsampling_factor)
    return avg_score
